...
export default function SuperAdminIndex() {
    ...
    const handleOpenEdit = (sa: SuperAdmin) => {
        setForm({
            id: sa.id,
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (isEdit && form.id) {
            router.put(`/super-admins/${form.id}`, form, {
                onSuccess: handleClose,
                onError: errs => setErrors(errs as Record<string, string>),
            });
        } else {
            router.post('/super-admins', form, {
                onSuccess: handleClose,
                onError: errs => setErrors(errs as Record<string, string>),
            });
        }
    ...
    const filtered = superAdminList.filter(sa => {
        const term = search.toLowerCase();
        return (
            sa.employee_id.toLowerCase().includes(term)  ||
            sa.full_name.toLowerCase().includes(term)    ||
            sa.email.toLowerCase().includes(term)        ||
            sa.access_level.toLowerCase().includes(term)
        );
    });
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <Dialog open={open} onOpenChange={setOpen}>
                <DialogContent className="max-w-md">
                    <DialogHeader>
                        <DialogTitle>{isEdit ? 'Update Super Admin' : 'Add Super Admin'}</DialogTitle>
                    </DialogHeader>
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div>
                            ...
                            <Input name="access_level" value={form.access_level} onChange={handleChange} required />
